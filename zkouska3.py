...
class Rectangle(Shape):
    def __init__(self, width, height):
        super().__init__("Rectangle")  # název tvaru
        self.width = width
        self.height = height

    def area(self):
        # plocha obdélníku: šířka * výška, zaokrouhleno na 1 desetinné místo
        return round(float(self.width * self.height), 1) 

# Circle dědí ze Shape
class Circle(Shape):
    def __init__(self, radius):
        super().__init__("Circle")  # název tvaru
        self.radius = radius

    def area(self):
        # plocha kruhu: π * r^2, zaokrouhleno na 1 desetinné místo
        return round(float(math.pi * self.radius ** 2), 1) 

    def __str__(self):
        # speciální výstup podle zadání
